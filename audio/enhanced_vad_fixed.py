    ...
    def _load_models(self):
        ...
        try:
            ...
            if self.cache_manager.is_model_cached(vad_model_name):
                ...
                cached_path = self.cache_manager.get_cached_model_path(vad_model_name)
                try:
                    self.vad_pipeline = Pipeline.from_pretrained(
                        str(cached_path),
                        use_auth_token=None
                    )
                    print(f"   ✅ Loaded VAD model from cache!")
                    vad_loaded = True
                except Exception as e:
                    logger.warning(f"Failed to load cached VAD model: {e}")
                    vad_loaded = False
            else:
                vad_loaded = False
            ...
            if self.config.enable_diarization:
                ...
                if self.cache_manager.is_model_cached(diarization_model_name):
                    ...
                    cached_path = self.cache_manager.get_cached_model_path(diarization_model_name)
                    try:
                        self.diarization_pipeline = Pipeline.from_pretrained(
                            str(cached_path),
                            use_auth_token=None
                        )
                        print(f"   ✅ Loaded diarization model from cache!")
                        diarization_loaded = True
                    except Exception as e:
                        logger.warning(f"Failed to load cached diarization model: {e}")
    ...
    def _load_model(self):
        ...
        try:
            ...
            if self.cache_manager.is_model_cached(model_name):
                print(f"✅ Found cached Silero VAD model")
                # Silero models don't support loading from custom paths easily
                # So we'll still download but this serves as a check
                pass
            
            print(f"\n🔄 Loading Silero VAD model")
            
            with tqdm(total=100, desc="📥 Downloading/Loading Silero model", unit="%") as pbar:
                ...
                # Mark as cached
                cache_dir = self.cache_manager.get_model_cache_dir(model_name)
                self.cache_manager.add_to_cache(
                    model_name,
                    cache_dir,
...
class FasterWhisperSTT:
    ...
    def _load_model(self):
        ...
        try:
            model_name = f"faster_whisper_{self.config.stt_model}"
            
            # Check if model is cached
            if self.cache_manager.is_model_cached(model_name):
                print(f"✅ Found cached faster-whisper model: {self.config.stt_model}")
                # Faster-whisper handles its own caching, this is just for tracking
                pass
            
            print(f"\n🔄 Loading faster-whisper model: {self.config.stt_model}")
            
            with tqdm(total=100, desc="📥 Downloading/Loading Whisper model", unit="%") as pbar:
